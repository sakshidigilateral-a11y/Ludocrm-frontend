...
type RankFilter = 'coins' | 'moves' | 'both';
...
const mapLeaderboardRow = (
  ...
  captain:
    activeTab === 'Team Rank'
      ...
      : item.mrName || 'Unknown',
  moves: getMoves(item.rank, item.metricValue),
  coins: Number(item.totalPoints || 0),
  isHighlighted: Boolean(item.isHighlighted),
});

const monthRangeForCurrentYear = (month: string) => {
...
const LeaderboardScreen: React.FC = () => {
  ...
  const FILTERS: { key: RankFilter; label: string }[] = [
    { key: 'coins', label: 'Coins' },
    { key: 'moves', label: 'Moves' },
    { key: 'both', label: 'Both' },
  ];

  const sortBy = activeFilter === 'moves' ? 'moves' : 'points';

  const leaderboardUrl = useMemo(() => {
    const endpoint =
      activeTab === 'Team Rank'
        ? '/api/flm/leaderboard/team/points'
        : '/api/flm/leaderboard/mr-points';
    ...
    const range = monthRangeForCurrentYear(activeMonth);
  ...
  const loadLeaderboard = useCallback(async () => {
    ...
    try {
      ...
      const json: LeaderboardApiResponse = await response.json();
        ...
        <View style={styles.barBorder}>
          ...
          >
            ...
            <Text style={[styles.headerText, styles.nameHeader]}>
              {activeTab === 'Team Rank' ? 'Team Name' : 'Captain'}
            </Text>
            {(activeFilter === 'coins' || activeFilter === 'both') && (
              <Text style={[styles.headerText, styles.metricHeader]}>
                Coins
              </Text>
            )}
            {(activeFilter === 'moves' || activeFilter === 'both') && (
              <Text style={[styles.headerText, styles.metricHeader]}>
                Moves
              </Text>
            )}
...
const styles = StyleSheet.create<Record<string, any>>({
  ...
  userRankRight: { alignItems: 'flex-end', gap: s(4) },
  userRankStat: { flexDirection: 'row', alignItems: 'center', gap: s(4) },
  userRankStatText: { color: '#fff', fontSize: s(12) },
  noRankText: { color: '#fff', fontSize: s(12), fontWeight: '700' },
  statIconSmall: { width: s(14), height: s(14), resizeMode: 'contain' },
  tableHeader: {
    flexDirection: 'row',
    borderRadius: s(25),
